...
var whatTimeIsIt = function (angle) {
	...
	console.log(
		`${('0' + String(hour)).slice(-2)}:${('0' + String(minute)).slice(-2)}`
	);
};
// whatTimeIsIt(50);
...
function handshake(n) {
	// amount of persons is given. should find an amount of handshakes
...
function duplicateEncode(word) {
	...
	for (let i = 0; i < letter.length; i++) {
		...
		if (obj[objKey] === undefined) {
			//если этот ключ еще не определен, значит такая буква еще не встречалась
			obj[objKey] = 1; // присваиваем ключу значение 1 (будет считать буквы)
		} else {
			//если не неандефайнд, значит эта буква уже встречалась
...
//Если у двух или трех сыновей одинаковые самые высокие оценки, нужно выбрать младшего.
{
	...
	function whoseBicycle(diary1, diary2, diary3) {
		...
		const excellentStudent = children.filter(
			person =>
				person.score ==
				Math.max(firstSon.score, secondSon.score, thirdSon.score)
		);
		let result = '';
	...
	function getCard() {
		...
		for (let i = 0; i < 5; i++) {
			...
			if (arrBingo.includes(number)) {
				//проверяем, чтобы исклюсить повторы
		...
		for (let i = 0; i < 5; i++) {
			...
			if (i === 2) {
				// проверка чтобы пропустить 1 ячейку
				continue;
	...
	function recycle(array) {
		...
		return materials.map(material =>
			array
				.filter(
					itemObject =>
						itemObject.material == material ||
						itemObject.secondMaterial == material
				)
				.map(obj => obj.type)
		);
	}
