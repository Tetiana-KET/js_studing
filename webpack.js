...
создадим файл index.js, который будет использоваться как входная точка, и импортируем в нем функции приложения:
import { sayHello, sayBye } from './application'

Настроим Webpack, чтобы он собирал единый файл с кодом приложения.

1.Для начала следует добавить Webpack в список зависимостей приложения:
  npm install --dev(-D) webpack webpack-cli

2.создать простой конфигурационный файл webpack.config.js (в корне проэкта)
